...
export default  async function WorkGrooming({ id }: WorkProps) {

  const data = await fetchAllPhotosData();

  const filteredData = data?.filter(item => item?.tags?.includes('Grooming')) || [];
  


  return (
    <WorkPage data={filteredData} id={id} path='/work/grooming'/>
  );
}