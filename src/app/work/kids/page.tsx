...
export default  async function WorkKids({ id }: WorkProps) {

  const data = await fetchAllPhotosData();

  const filteredData = data?.filter(item => item?.tags?.includes('Kids')) || [];
  


  return (
    <WorkPage data={filteredData} id={id} path='/work/kids'/>
  );
}