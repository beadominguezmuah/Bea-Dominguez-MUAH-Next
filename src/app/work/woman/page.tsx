...
export default  async function WorkWoman({ id }: WorkProps) {

  const data = await fetchAllPhotosData();

  const filteredData = data?.filter(item => item?.tags?.includes('Woman')) || [];
  


  return (
    <WorkPage data={filteredData} id={id} path='/work/woman'/>
  );
}